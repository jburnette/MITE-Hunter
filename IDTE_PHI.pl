...
=head1 USAGE
 ...
 tblastn_copy_finder.pl <options> <specification> <default>
 
 -i or --input      Input file 
 -q or --query      Query file
 -s or --subject    Subject File
 -e or --maxevalue  Max E-value (default 0.01)
 -M or --minprot    Min Protein
 -d or --intron     Max Intron Distance (8000 default)
 -I or --idtag      ID Tag
 -n or --maxoutput  Max output
 -l or --flankleft  Left Flank length (60)
 -r or --flankright Right Flank length (60)
 -s or --subregion  Sub region length
 -L or --maxoverlap Max overlap (30)
 -o or --output     Output
 -h or --help       Print help

=cut

# Jason Stajich changes
# 2010-11-28
# Changes
# 1.  Use strict
# 2. Getopt::Long
...
my $Input = '';
my $Query_File = "soymar1.aa"; # sensible defaults?
my $Sbjct_File = ''; # sensible defaults?
my $Max_Evalue = 0.01;
my $Min_Pro = 0.7;
my $Max_Intron = 8000;
my $ID_Tag = 'ZMar'; # Sensible default?
my $Max_Output = 30;
my $Flank_L = 60;
my $Flank_R = 60;
my $Sub_Region_Len = 0;
...
GetOptions(
	...
	'D|subject:s'          => \$Sbjct_File,
	'e|maxevalue:f'	       => \$Max_Evalue,
	'M|minpro:f'           => \$Min_Pro,
	'd|intron|maxintron:i' => \$Max_Intron,
	'I|idtag|ID:s'         => \$ID_Tag,
	'n|maxoutput:i'        => \$Max_Output,
	'l|left|flankleft:i'   => \$Flank_L,
	'r|right|flankright:i' => \$Flank_R,
	's|subregion:i'        => \$Sub_Region_Len,
	'L|maxoverlap:i'       => \$Max_Overlap,
	'o|output:s'           => \$Output,
	'h|help'               => sub { usage() },
	);
...
if($Max_Evalue =~ /e/) {
 ...
 $Max_Evalue = sprintf("%e",$Max_Evalue);
}
...
# my $qf = Bio::SeqIO->new(-format => 'fasta', -file => $Query_File);
# while( my $seq = $qf->next_seq ) { $Query_Name_Seq{$seq->id} = $seq->seqi; $Query_Length{$seq->id} = $seq->length }
my (%Query_Name_Seq,%Query_Length);
{ 
open(QF, $Query_File)||die "Cannot open $Query_File: $!\n";
my $Name;
while(<QF>) {
	if (/^>(\S+)/) {
		$Name = $1;
		...
		s/\s+$//;
		$Query_Name_Seq{$Name} .= $_;
	}
}
close(QF);

for my $id (keys(%Query_Name_Seq)) {
	$Query_Length{$id} = length($Query_Name_Seq{$id});
}
}
#-----------------Load blast result---------------------------------
my (%Query_Sbjcts,%Query_Sbjct_Matches);
...
for my $Query ( keys %Query_Sbjcts) {  
	my @Matched_Sbjcts = split(/ /, $Query_Sbjcts{$_});
	for my $Sbjct (@Matched_Sbjcts) {
		my %B_E_For         = ();
		my %B_E_Rev         = ();
		my @S_Begins_Plus   = ();
		my @S_Begins_Minus  = ();
		...
		my %First_Score     = ();
		my $Plus_Match_Num  = 0;
		my $Minus_Match_Num = 0;
		
		# You should be using array refs here I think
		my @Match_Pairs = split(/   /, $Query_Sbjct_Matches{$Query." ".$Sbjct});

		for my $pair (@Match_Pairs) {

			my ($Query_Info, $Sbjct_Info)  = split(/  /, $pair);             # $Match_Info = $Q_Begin." ".$Q_Match." ".$Q_End."  ".$S_Begin." ".$S_Match." ".$S_End."   ";
			my ($Q_Begin, $Q_Match, $Q_End) = split(/ /, $Query_Info);    # $Q_Begin." ".$Q_Match." ".$Q_End
			my ($S_Begin, $S_Match, $S_End, $Frame, $Score) = split(/ /, $Sbjct_Info);
...
my $Num = 0;
for my $info (keys(%Query_Sbjct_Copy_Infor)) {
#	print "$_ -> $Query_Sbjct_Copy_Infor{$_}\n";
	my ($Query, $Sbjct, $Start_Loc) = split(/ /, $info);
	my $Query_Len = $Query_Length{$Query};
 	my $Copy_Score = 0;
	my $Copy_Len   = 0;
	my @Matches = split(/  /, $Query_Sbjct_Copy_Infor{$info});
	
	my @Query_Match_Region = ();
	for(my $i = 0; $i < $Query_Len; $i++) {
		$Query_Match_Region[$i] = 0;
	}
	# Arrayref would be better here
	for my $m (@Matches) {
		my ($Q_Begin, $Q_Match, $Q_End, $S_Begin, $S_Match, $S_End, $Frame, $Score) = split(/ /, $_);
	...
	my $Stop_Loc = $S_Begin < $S_End ? $S_End : $S_Begin;
...
# I don't understand why this function exists? Why are you reinventing <=> ? 
# --JasonS
	my($E1, $E2) = @_;
